...
// Function: Renders a tabular interface for configuring ranking multipliers for scholarly historical debates
// Output: Injects an academic weight-configuration table into the specified container

window.renderEditAcademicWeights = function(containerId) {
    ...
    const html = `
        <div class="admin-card" id="edit-academic-weights-card">
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid var(--color-border); margin-bottom: var(--space-4); padding-bottom: var(--space-2);">
                <h2 style="border: none; margin: 0; padding: 0; font-family: var(--font-serif);">RANKED LIST: Academic Debates</h2>
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left; font-family: var(--font-mono); font-size: var(--text-sm);">
                ...
                <tbody>
                    <tr style="border-bottom: 1px solid #eee;">
                        <td style="padding: var(--space-3) var(--space-2);">historicity-of-miracles</td>
                        <td style="padding: var(--space-3) var(--space-2); color: var(--color-primary); font-weight: bold;">800</td>
                        <td style="padding: var(--space-2);"><input type="number" step="0.1" value="1.5" style="width: 80px; padding: var(--space-1); border: 1px solid #ccc;"> x</td>
                    </tr>
                </tbody>
            </table>
        </div>
