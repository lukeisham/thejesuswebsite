 ...
 */
Edges.validateConnection = function (sourceId, targetId, relationshipType, existingEdges) {
  if (sourceId === targetId) {
    return "An evidence node cannot connect to itself.";
  }

  if (VALID_TYPES.indexOf(relationshipType) === -1) {
    return "Invalid relationship type. Must be one of: " + VALID_TYPES.join(", ");
  }
...
Edges.createEdgeElement = function (edge, sourceNode, targetNode) {
  ...
  var line = window.AdminArborCanvas.createEdgeLine(x1, y1, x2, y2, "admin-arbor-edge");
  g.appendChild(line);
...
Edges.onEdgeClick = function (edge) {
  ...
  if (!confirm(
    "Delete the \u201C" + edge.relationship_type + "\u201D connection from \u201C" +
...
Edges.onEdgeDragUp = async function (e) {
  ...
  // Validate
  var error = Edges.validateConnection(sourceNode.id, targetNode.id, relationshipType, edges);
  if (error) {
...
Edges.repositionEdgesForNode = function (nodeId, newX, newY) {
  ...
  for (var i = 0; i < edgeGroups.length; i++) {
    ...
    var edge = null;
    for (var j = 0; j < edges.length; j++) {
      if (edges[j].id === edgeId) { edge = edges[j]; break; }
    }
    if (!edge) continue;
    ...
    var x1 = sourceNode ? (sourceNode.arbor_x || 0) : Number(line.getAttribute("x1"));
    var y1 = sourceNode ? (sourceNode.arbor_y || 0) : Number(line.getAttribute("y1"));
    var x2 = targetNode ? (targetNode.arbor_x || 0) : Number(line.getAttribute("x2"));
    var y2 = targetNode ? (targetNode.arbor_y || 0) : Number(line.getAttribute("y2"));

    if (edge.source_id === nodeId) { x1 = newX; y1 = newY; }
    if (edge.target_id === nodeId) { x2 = newX; y2 = newY; }
