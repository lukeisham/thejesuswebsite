...
function statusBadge(publishedDraft) {
  return {
    className: 'admin-badge ' + (publishedDraft ? 'admin-badge--published' : 'admin-badge--draft'),
...
test('Admin.formatDate returns readable date with year', function () {
  ...
  assert.ok(result.includes('2024'), 'Expected result to contain "2024", got: ' + result);
});
...
test('Admin.statusBadge(1) builds published badge', function () {
  ...
  assert.ok(badge.className.includes('admin-badge--published'), 'Expected published class');
  assert.ok(!badge.className.includes('admin-badge--draft'), 'Should not include draft class');
...
test('Admin.statusBadge(0) builds draft badge', function () {
  ...
  assert.ok(badge.className.includes('admin-badge--draft'), 'Expected draft class');
  assert.ok(!badge.className.includes('admin-badge--published'), 'Should not include published class');
...
test('computeSparkline produces normalized y values within height', function () {
  ...
  assert.ok(pairs[0].endsWith(',50.0'), 'Min value should map to bottom height, got: ' + pairs[0]);

  // Last point: y close to 0 (top for max)
  assert.ok(pairs[2].endsWith(',0.0'), 'Max value should map to top 0, got: ' + pairs[2]);
});
...
test('computeSparkline with all-zero values maps all to bottom', function () {
  ...
  pairs.forEach(function (p) {
    assert.ok(p.endsWith(',30.0'), 'All zero values should map to bottom (30), got: ' + p);
  });
});
