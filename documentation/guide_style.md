...
| **Verse Builder** | 3-dropdown + input row (book / chapter / verse); rendered as chips in a flex-wrap container; inline validation hint below inputs | `.verse-builder__select`, `.verse-builder__chips`, `.verse-builder__hint` (error: `--color-status-error`, `--text-xs`, `--font-mono`), `.chip` |
