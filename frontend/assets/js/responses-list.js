/**
 * Responses list page: fetch paginated debate responses, render cards,
 * infinite scroll, and sessionStorage caching.
 *
...
const STORAGE_KEY_ITEMS = 'responses_list_items';
const STORAGE_KEY_SCROLL = 'responses_list_scroll';
...
async function loadPage() {
  ...
  if (!data || data.length === 0) {
    if (allItems.length === 0) {
      ...
      if ($end) $end.textContent = `All ${total} response${total !== 1 ? 's' : ''} loaded`;
    }
    return;
  }
...
function renderCards(items) {
  ...
  items.forEach((item) => {
    const date = item.response_date || item.created_at;
    const dateStr = date
      ? new Date(date).toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' })
      : '';
    const byline = item.response_author
      ? `${item.response_author}${dateStr ? ` · ${dateStr}` : ''}`
      : dateStr;

    const cardHTML = renderCard({
      title: item.response_title || 'Untitled',
      description: byline || '',
      url: `/debate/responses/${encodeURIComponent(item.slug || '')}`,
      badges: [],
    });
    ...
    if (cardEl) {
      ...
      if (typeBadge) {
        ...
        respBadge.textContent = 'Response';
        typeBadge.appendChild(respBadge);
      }
