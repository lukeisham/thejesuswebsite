...
function renderListView() {
    ...
    if (titleEl) {
        if (searchParam) {
            ...
            titleEl.textContent = 'Category: ' + (categoryParam.charAt(0).toUpperCase() + categoryParam.slice(1));
        } else if (eraParam) {
    ...
    if (records.length === 0) {
        listEl.innerHTML = '<li class="list-row-item"><p>No records found matching your criteria.</p></li>';
    } else {
        var html = records.map(function(record) {
            ...
            // Determine base URL (if in subfolder like 'resources/', use '../record.html')
            var isSubfolder = window.location.pathname.includes('/resources/');
            var baseRecordUrl = isSubfolder ? '../record.html' : 'record.html';

            // Format depending on presence of verse reference (matches Guide Appearance 2.3 vs 2.4/others)
            if (verseText) {
                return [
                    '<li class="resource-row">',
                    '    <a href="' + baseRecordUrl + '?slug=' + encodeURIComponent(record.slug) + '" class="resource-title">' + record.title + '</a>',
                    '    <div>',
                ...
                var metaHtml = metaText ? '<strong style="font-family:var(--font-mono);font-size:var(--text-xs);margin-right:8px;">[' + metaText + ']</strong>' : '';
                return [
                    '<li class="list-row-item">',
                    '    <div class="list-row-content">',
                    '        <a href="' + baseRecordUrl + '?slug=' + encodeURIComponent(record.slug) + '" class="list-row-title">' + record.title + '</a>',
    ...
    if (paginationEl) {
        if (!searchParam && (pageParam > 1 || hasNext)) {
            ...
            if (pageParam > 1) {
                ...
                navHtml += '<a href="?' + prevParams.toString() + '" class="pagination-btn">← Previous</a>';
            }
            if (hasNext) {
                var nextParams = new URLSearchParams(window.location.search);
                nextParams.set('page', pageParam + 1);
                navHtml += '<a href="?' + nextParams.toString() + '" class="pagination-btn">Next →</a>';
            }
