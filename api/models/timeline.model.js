...
// The three broad eras, in narrative order.
...
function getTimelineEvents({ timeline_era } = {}) {
    ...
    return rows.sort((a, b) => {
        const eraDelta = ordinalOf(ERA_ORDER, a.timeline_era) - ordinalOf(ERA_ORDER, b.timeline_era);
        if (eraDelta !== 0) return eraDelta;
        return ordinalOf(PERIOD_ORDER, a.timeline_period) - ordinalOf(PERIOD_ORDER, b.timeline_period);
    });
}

/** Convenience wrapper for a single era (beginning | middle | end). */
