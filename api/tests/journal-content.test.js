...
const MIGRATION_PATH = path.resolve(__dirname, '..', '..', 'database', 'migrations', '003_journal_article_metadata.sql');
...
describe('responses: composite CRUD', () => {
  ...
  test('createComposite with breakouts and pictures', () => {
    ...
    const created = responseModel.createComposite({
      ...
      pictures: [
        { image_path: '/img.jpg', caption: 'Test Image' },
      ],
  ...
  test('updateComposite returns undefined for non-existent id', () => {
    assert.equal(responseModel.updateComposite(99999, { response_title: 'Ghost' }), undefined);
  });
