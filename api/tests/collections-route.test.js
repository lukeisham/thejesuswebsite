// Collections route tests — uses node:test + node:assert.
// Minimum coverage proving the API-3 registry conversion: a 404 case and the
// "slug and title are required" 400 case both return { error: { code, message } }.
...
function createApp() {
  ...
  const routePath = require.resolve("../routes/collections");
...
describe("POST /collections", () => {
  test("returns 400 with E-INPUT-001 when slug and title are missing", async () => {
    const result = await request(createApp(), {
      method: "POST",
      path: "/collections",
      body: {},
