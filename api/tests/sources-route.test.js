// Sources (MLA bibliography) route tests — uses node:test + node:assert.
// Minimum coverage proving the API-3 registry conversion: a 404 case and the
// "No valid fields provided" 400 case both return { error: { code, message } }.
...
function createApp() {
  ...
  const routePath = require.resolve("../routes/sources");
...
describe("POST /sources", () => {
  test("returns 400 with E-INPUT-001 when no valid fields are provided", async () => {
    const result = await request(createApp(), {
      method: "POST",
      path: "/sources",
      body: {},
