...
def setup_logger(caller_file, is_test=False):
    ...
    LOG_DIR = os.path.join(_ROOT_DIR, 'tests', 'reports') if is_test else os.path.join(_ROOT_DIR, 'logs')
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            RotatingFileHandler(log_path, maxBytes=5*1024*1024, backupCount=2) # 5MB per file
        ]
    )
