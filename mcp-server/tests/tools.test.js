...
describe('getItemBySlug', () => {
  it('builds correct URL with slug', async () => {
    ...
    assert.equal(
      apiRequest.recordedPath(),
      '/evidence/baptism-of-jesus',
    );
    assert.equal(result.content[0].text, JSON.stringify({ title: 'Baptism', slug: 'baptism' }, null, 2));
  });
...
describe('getBlogPostBySlug', () => {
  it('builds correct URL with slug', async () => {
    ...
    await getBlogPostBySlug.handler(
      { slug: 'debate-update' },
      apiRequest,
    );
...
describe('getNewsArticleBySlug', () => {
  it('builds correct URL with slug', async () => {
    ...
    assert.equal(
      apiRequest.recordedPath(),
      '/news-articles/qumran-discovery',
    );
  });
...
describe('getTimelineEvents', () => {
  ...
  it('appends timeline_era query param for valid era', async () => {
    ...
    assert.equal(
      apiRequest.recordedPath(),
      '/timeline?timeline_era=beginning',
    );
  });
