...
async function displayBlogPostsList() {
  try {
    ...
    const posts = Array.isArray(data) ? data : (data.records || data.blogposts || data.results || []);
    ...
    console.error('[display_blog_posts_data] Failed to load blog post list:', err);
    if (typeof window.surfaceError === 'function') {
      window.surfaceError('Error: Unable to load blog posts list. Please refresh and try again.');
    }
  }
}
...
async function loadBlogPostContent(recordId, title) {
  ...
  try {
    ...
    if (typeof window.setMarkdownContent === 'function') {
      window.setMarkdownContent(post.blogposts || post.markdown_content || post.content || '');
    }
    ...
    if (metadataJson) {
      metadataJson.value = post.metadata_json
        ? (typeof post.metadata_json === 'string' ? post.metadata_json : JSON.stringify(post.metadata_json, null, 2))
        : '';
    }
    if (createdAt) {
    ...
    if (typeof window.loadEditBibliography === 'function') {
      try {
        ...
        console.warn('[display_blog_posts_data] Failed to load bibliography:', err);
      }
    }
    ...
    if (typeof window.renderEditLinks === 'function') {
      try {
        window.renderEditLinks('blog-context-links-container', post.context_links || []);
      } catch (err) {
        console.warn('[display_blog_posts_data] Failed to load context links:', err);
      }
    }
    ...
    if (typeof window.renderEditPicture === 'function') {
      try {
        ...
        console.warn('[display_blog_posts_data] Failed to wire picture handler:', err);
      }
    }
    ...
    if (typeof window.renderMetadataFooter === 'function') {
      try {
        ...
        console.warn('[display_blog_posts_data] Failed to wire metadata footer:', err);
      }
    }
    ...
    if (typeof window.surfaceError === 'function') {
      window.surfaceError("Error: Unable to load post '" + (title || recordId) + "'. Please try again.");
    }
  }
}
...
function _highlightActiveItem(recordId) {
  ...
  const allItems = document.querySelectorAll('.blog-sidebar-list__item--active');
  ...
  if (recordId) {
    const activeItem = document.querySelector('.blog-sidebar-list__item[data-record-id="' + CSS.escape(recordId) + '"]');
    if (activeItem) {
