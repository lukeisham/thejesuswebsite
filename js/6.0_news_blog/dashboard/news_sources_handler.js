...
async function publishNewsSources() {
    const records = window._newsSourcesModuleState &&
    ...
    for (var i = 0; i < records.length; i++) {
        ...
        try {
            var response = await fetch('/api/admin/records/' + record.id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            console.error('[news_sources_handler] Publish failed for ' + (record.title || record.id) + ':', err);
            failCount++;
        }
    }

    // Surface result to the Status Bar
    if (typeof window.surfaceError === 'function') {
        if (failCount === 0) {
            window.surfaceError('All ' + successCount + ' news sources published successfully.');
        } else {
            window.surfaceError(
                'Published ' + successCount + ' sources. ' + failCount + ' failed. Please try again.'
            );
        }
    }
...
function _buildNewsSourceRow(record) {
    ...
    try {
        if (record.news_sources) {
            var sourceData = typeof record.news_sources === 'string'
...
function _selectNewsSourceRow(rowEl, record) {
    ...
    try {
        if (record.news_search_term) {
            var termsData = typeof record.news_search_term === 'string'
                ? JSON.parse(record.news_search_term)
                : record.news_search_term;
            if (Array.isArray(termsData)) {
                searchKeywords = termsData;
            } else if (typeof termsData === 'string') {
                searchKeywords = termsData.split(',').map(function (t) { return t.trim(); }).filter(Boolean);
        ...
        if (record.news_search_term && typeof record.news_search_term === 'string') {
            searchKeywords = record.news_search_term.split(',').map(function (t) { return t.trim(); }).filter(Boolean);
        }
    }
    ...
    if (window._newsSourcesModuleState) {
        ...
        window._newsSourcesModuleState.activeMeta = record.meta || '';
