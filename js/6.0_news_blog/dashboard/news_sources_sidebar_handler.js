...
function populateNewsSourcesSidebar(record) {
    ...
    if (metadataFooter && typeof metadataFooter._setMetadataDisplay === 'function') {
        metadataFooter._setMetadataDisplay({
            metadata_json: record.meta || '',
    ...
    if (sharedMetadataJson) {
        sharedMetadataJson.value = record.meta || '';
    }
...
async function _handleSaveUrl() {
    ...
    if (!state || !state.activeRecordId) {
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: No news source selected. Please select a source first.');
        }
        return;
    }
    ...
    try {
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError("Error: Failed to update news source '" + title + "'. Please try again.");
        }
    }
}
...
async function _handleSaveMetadata() {
    ...
    try {
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Failed to save metadata for the selected record.');
        }
    }
}
...
async function _handleAutoGenSnippet() {
    ...
    if (!state || !state.activeRecordId) {
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Snippet generation failed. No source selected.');
        }
        return;
    }
    ...
    try {
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Snippet generation failed. Please try again or enter manually.');
        }
    } finally {
        if (btn) {
            btn.disabled = false;
...
async function _handleAutoGenMeta() {
    ...
    if (!state || !state.activeRecordId) {
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Metadata generation failed. No source selected.');
        }
        return;
    }
    ...
    try {
        ...
        var data = await response.json();
        var metaJson = data.metadata_json || data.meta || '';

        // Populate the meta input
        var metaInput = document.getElementById('news-sources-meta-input');
        if (metaInput) {
            metaInput.value = typeof metaJson === 'string' ? metaJson : JSON.stringify(metaJson);
        }
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Metadata generation failed. Please try again or enter manually.');
        }
    } finally {
        if (btn) {
            btn.disabled = false;
