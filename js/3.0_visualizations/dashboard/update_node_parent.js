...
async function updateNodeParent(childId, newParentId) {
    ...
    if (!childData) {
        console.error("[update_node_parent] Node not found in diagramNodes:", childId);
        return false;
    }
    ...
    if (newParentId && _isAncestor(childId, newParentId, nodesMap)) {
        const message =
            `Error: Cannot re-parent '${title}' — this would create a circular loop in the tree.`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const serverDetail = errorBody.detail || `${response.status} ${response.statusText}`;
            ...
            const message =
                `Error: Failed to save new parent for '${title}'. The diagram has been reset to its previous state.`;
            if (typeof window.surfaceError === "function") {
                window.surfaceError(message);
            }
            console.error(
