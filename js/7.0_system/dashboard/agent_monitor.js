...
function _renderSummaryBar(runs) {
    ...
    if (tokensEl) tokensEl.textContent = `Tokens Today: ${tokenTotal.toLocaleString()}`;
...
function _renderTable(runs) {
    ...
    runs.forEach(function (run) {
        ...
        tdTokens.textContent = run.tokens_used !== null && run.tokens_used !== undefined
...
function _selectRun(run) {
    ...
    if (labelEl) {
        labelEl.textContent =
            '(' + (run.record_slug || 'unknown') + ' @ ' + _formatTime(run.started_at) + ')';
    }
...
function _clearTraceReasoning() {
    ...
    if (contentEl) {
        contentEl.style.fontStyle = '';
        contentEl.style.color = '';
        contentEl.textContent = 'Select an agent run above to view its chain-of-thought reasoning.';
    }
    if (labelEl) {
