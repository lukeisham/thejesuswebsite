...
async function _handleSaveDraft() {
  ...
  if (!recordId) {
    if (typeof window.surfaceError === 'function') {
      window.surfaceError('Error: No document selected. Please select a document from the sidebar first.');
    }
    return;
  }
  ...
  try {
    ...
    if (typeof window.displayEssayHistoriographyList === 'function') {
      await window.displayEssayHistoriographyList(window._essayModuleState.mode);
    }
...
function _collectEditorData() {
  ...
  if (typeof window.collectEditBibliography === 'function') {
    try {
      ...
      console.warn('[document_status_handler] Failed to collect bibliography:', err);
  ...
  if (typeof window.collectEditLinks === 'function') {
    try {
      ...
      console.warn('[document_status_handler] Failed to collect context links:', err);
