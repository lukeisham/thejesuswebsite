...
async function displayEssayHistoriographyList(mode) {
  ...
  try {
    ...
    console.error(
      "[essay_historiography_data_display] Failed to load document list:",
      err,
    );
  ...
  try {
    ...
    if (typeof window.loadEditBibliography === "function") {
      try {
        ...
        console.warn(
          "[essay_historiography_data_display] Failed to load bibliography:",
          err,
        );
      }
    }
    ...
    if (typeof window.renderEditLinks === "function") {
      try {
        window.renderEditLinks(
          "essay-context-links-container",
          doc.context_links || [],
        );
      } catch (err) {
        console.warn(
          "[essay_historiography_data_display] Failed to load context links:",
          err,
        );
      }
    }
    ...
    if (typeof window.renderEditPicture === "function") {
      try {
        ...
        console.warn(
          "[essay_historiography_data_display] Failed to wire picture handler:",
          err,
        );
      }
    }
    ...
    if (typeof window.populateMetadataWidget === "function") {
      try {
        ...
        console.warn(
          "[essay_historiography_data_display] Failed to populate metadata widget:",
          err,
        );
      }
    }
    ...
    console.error(
      "[essay_historiography_data_display] Failed to load document:",
      err,
    );
    if (typeof window.surfaceError === "function") {
      window.surfaceError(
...
function _highlightActiveItem(recordId) {
  ...
  const allItems = document.querySelectorAll(
    ".essay-sidebar-list__item--active",
  );
  ...
  if (recordId) {
    const activeItem = document.querySelector(
      '.essay-sidebar-list__item[data-record-id="' +
        CSS.escape(recordId) +
        '"]',
    );
    if (activeItem) {
