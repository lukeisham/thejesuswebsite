...
function initMarkdownEditor(initialContent) {
  ...
  if (!textarea) {
    console.warn('[markdown_editor] #markdown-textarea not found — editor not initialised.');
    return;
  }
  if (!preview) {
    console.warn('[markdown_editor] #markdown-preview not found — preview disabled.');
  }
...
function _renderPreview(markdownText, previewEl) {
  ...
  try {
    ...
    if (typeof window.surfaceError === 'function') {
      window.surfaceError('Error: Markdown preview failed. Check document content for invalid syntax.');
    }
    previewEl.innerHTML = '<p class="state-error__label">Preview error — check console for details.</p>';
  }
}
