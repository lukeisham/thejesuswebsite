...
async function displayWikipediaList() {
    ...
    try {
        ...
        const wikipediaRecords = allRecords.filter(function (rec) {
            return rec.wikipedia_rank !== null &&
...
function _buildWikipediaRow(record, index) {
    ...
    try {
        if (record.wikipedia_link) {
            const linkData = typeof record.wikipedia_link === "string"
...
function _selectWikipediaRow(rowEl, record) {
    ...
    try {
        if (record.wikipedia_search_term) {
            const termsData = typeof record.wikipedia_search_term === "string"
            ...
            if (Array.isArray(termsData)) {
                ...
                searchTerms = termsData.split(",").map(function (t) { return t.trim(); }).filter(Boolean);
        ...
        if (record.wikipedia_search_term && typeof record.wikipedia_search_term === "string") {
            searchTerms = record.wikipedia_search_term.split(",").map(function (t) { return t.trim(); }).filter(Boolean);
        }
    }
    ...
    window._wikipediaModuleState.activeRecordMeta = record.meta || "";
