...
async function _handleSaveWeight() {
    ...
    if (!state.activeRecordId) {
        if (typeof window.surfaceError === "function") {
            window.surfaceError("No record selected. Select a record to edit its weight.");
        }
        return;
    }
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Weight saved for '" + state.activeRecordTitle + "'. Record set to draft.");
        }
    } catch (err) {
        console.error("[wikipedia_sidebar] Weight save failed:", err);
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Failed to save Wikipedia weight for '" + state.activeRecordTitle + "'.");
        }
    }
}
...
async function _handleAddTerm() {
    ...
    if (!state.activeRecordId) {
        if (typeof window.surfaceError === "function") {
            window.surfaceError("No record selected. Select a record to add search terms.");
        }
        return;
    }
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Search term added to '" + state.activeRecordTitle + "'. Record set to draft.");
        }
    } catch (err) {
        console.error("[wikipedia_sidebar] Add term failed:", err);
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Failed to save search terms for '" + state.activeRecordTitle + "'.");
        }
    }
}
...
async function _handleRemoveTerm(index) {
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Search term removed from '" + state.activeRecordTitle + "'. Record set to draft.");
        }
    } catch (err) {
...
async function _handleSaveMetadata() {
    ...
    const newSnippet = snippetInput ? snippetInput.value.trim() : state.activeRecordSnippet;
    ...
    if (newSnippet === state.activeRecordSnippet &&
    ...
    try {
        ...
        if (newMeta !== state.activeRecordMeta) payload.meta = newMeta;
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Metadata saved for '" + state.activeRecordTitle + "'. Record set to draft.");
        }
    } catch (err) {
        console.error("[wikipedia_sidebar] Metadata save failed:", err);
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Failed to save metadata for '" + state.activeRecordTitle + "'.");
        }
    }
}
...
async function _handleAutoGenSnippet() {
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Snippet auto-generated for '" + state.activeRecordTitle + "'.");
        }
    } catch (err) {
        console.error("[wikipedia_sidebar] Auto-gen snippet failed:", err);
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Failed to auto-generate snippet for '" + state.activeRecordTitle + "'.");
        }
    } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Auto-gen Snippet"; }
    }
}
...
async function _handleAutoGenMeta() {
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Metadata auto-generated for '" + state.activeRecordTitle + "'.");
        }
    } catch (err) {
        console.error("[wikipedia_sidebar] Auto-gen meta failed:", err);
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Failed to auto-generate metadata for '" + state.activeRecordTitle + "'.");
        }
    } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Auto-gen Meta"; }
    }
}
...
async function _handleRecalculateRecord() {
    ...
    if (!state.activeRecordId) {
        if (typeof window.surfaceError === "function") {
            window.surfaceError("No record selected. Select a record to recalculate.");
        }
        return;
    }
    ...
    try {
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Recalculate triggered for '" + state.activeRecordTitle + "'. Reloading...");
        }
        ...
        if (typeof window.surfaceError === "function") {
            window.surfaceError("Error: Recalculate failed for '" + state.activeRecordTitle + "'. Pipeline did not respond.");
        }
    } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Recalculate This Record"; }
    }
}
