...
Object.defineProperty(window, '_recordTitle', {
    get: function () { return window._wikipediaModuleState.activeRecordTitle; },
    configurable: true
});
...
async function renderWikipedia() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to load Wikipedia editor template (HTTP ' + response.status + ')');
        }
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Unable to load the Wikipedia editor. Please refresh and try again.');
        }
        return;
    }
...
function _wireActionButtons() {
    ...
    const btnRecalculateAll = document.getElementById('btn-wikipedia-recalculate-all');
...
async function _recalculateAllRecords() {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errData.error || 'Agent run failed with status ' + response.status);
        }
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Failed to trigger Wikipedia pipeline. Please try again.');
        }
    }
}
