...
function insertChallengeResponse() {
  ...
  if (!parentSlug) {
    if (typeof window.surfaceError === "function") {
      window.surfaceError(
        "Please select a challenge before inserting a response.",
      );
    }
    return;
  }
    ...
    newBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        if (typeof window.loadModule === "function") {
          // Store the response ID so the single-record editor can load it
          window._selectedRecordId = data.id;
        ...
        if (typeof window.surfaceError === "function") {
          window.surfaceError(
            "Error: Failed to create response for challenge '" +
              parentTitle +
              "'.",
          );
        }
