...
//           textarea for auto-save on change.
// Output:  Interactive weighting sidebar with auto-save on every change.
...
/* -----------------------------------------------------------------------------
   ...
   Called when the Academic/Popular toggle switches. Resets the weighting
   list to defaults for the new mode.
...
/* -----------------------------------------------------------------------------
   ...
   into the sidebar textarea and auto-saves on change.
...
function loadChallengeSearchTerms(challenge) {
    ...
    const searchTermCol = mode === 'academic'
    ...
    // Parse and display search terms
    ...
    var weightCol = mode === 'academic'
        ? 'academic_challenge_weight'
        : 'popular_challenge_weight';
    var weightRaw = challenge[weightCol] || '{}';

    try {
        var weights = (typeof weightRaw === 'string') ? JSON.parse(weightRaw) : weightRaw;
...
function _renderWeightingList() {
    ...
    if (criteria.length === 0) {
        criteria = JSON.parse(JSON.stringify(DEFAULT_WEIGHTS[mode] || DEFAULT_WEIGHTS.academic));
        window._challengeModuleState.weightingCriteria = criteria;
    }
...
function _wireNewWeightForm() {
    ...
    addBtn.addEventListener('click', function () {
        ...
        var value = (valueInput && valueInput.value) ? parseFloat(valueInput.value) : 0;

        if (!name) {
            if (typeof window.surfaceError === 'function') {
                window.surfaceError('Please enter a name for the new weighting criterion.');
            }
            return;
        }
...
async function _autoSaveWeights() {
    ...
    try {
        var response = await fetch('/api/admin/records/' + encodeURIComponent(slug), {
            method: 'PUT',
        ...
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Failed to save weighting changes. Please try again.');
        }
    }
}
...
async function _autoSaveSearchTerms() {
    ...
    try {
        ...
        console.error('[challenge_weighting_handler] Search term save failed:', err);
        if (typeof window.surfaceError === 'function') {
            window.surfaceError('Error: Failed to save search terms. Please try again.');
        }
    }
}
