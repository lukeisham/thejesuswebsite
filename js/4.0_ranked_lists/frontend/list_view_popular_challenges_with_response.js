// Trigger: DOMContentLoaded -> renderPopularChallengesWithResponses('popular-challenge-list-container')
// Function: fetchPopularChallengesWithResponses fetches challenges + responses from live API,
//           matches responses to challenges by challenge_id FK, and injects paired HTML rows
// Output:  Ranked list of popular challenges, each with a response sub-card when available
...
function renderPopularChallengesWithResponses(containerId) {
  ...
  Promise.all([challengesPromise, responsesPromise])
    ...
    .catch(function (err) {
      console.error("Popular challenges with responses error:", err);
      container.innerHTML =
        '<div class="empty-state text-center py-12">' +
