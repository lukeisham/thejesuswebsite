...
function collectAllFormData() {
    ...
    payload.id = _getValue('record-id');
    ...
    if (typeof window.collectDescription === 'function') {
        const description = window.collectDescription('description-editor-container');
    ...
    if (typeof window.collectVerses === 'function') {
        payload.primary_verse = JSON.stringify(window.collectVerses('primary-verse-container'));
        payload.secondary_verse = JSON.stringify(window.collectVerses('secondary-verse-container'));
    }
...
/* -----------------------------------------------------------------------------
   ...
   Collects all form data, sets status to 'draft', and PUTs to the API.
----------------------------------------------------------------------------- */
async function handleSaveDraft() {
    ...
    if (!recordId) {
        _surfaceError("Error: Cannot save — no record ID found.");
        return false;
    }
...
/* -----------------------------------------------------------------------------
   ...
   Collects all form data, sets status to 'published', and PUTs to the API.
----------------------------------------------------------------------------- */
async function handlePublish() {
    ...
    if (!recordId) {
        _surfaceError("Error: Cannot publish — no record ID found.");
        return false;
    }
    ...
    try {
        ...
        _surfaceError(`Error: Failed to publish record '${title}'. Check required fields.`);
        return false;
    }
}
...
async function handleDelete() {
    ...
    try {
        ...
        _surfaceError(`Error: Failed to delete record '${title}'. Please try again.`);
        return false;
    }
}
