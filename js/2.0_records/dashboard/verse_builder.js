...
function renderVerseBuilder(containerId, verses) {
    ...
    if (!container) {
        if (typeof window.surfaceError === 'function') {
            ...
            window.surfaceError(`Error: Unable to parse verse references for '${title}'.`);
        }
        return;
    }
    ...
    container.innerHTML = `
        <div class="verse-builder">
            ...
            <div class="verse-builder__inputs">
                <select class="verse-builder__select" data-verse-book>
    ...
    addButton.addEventListener('click', function () {
        ...
        if (!book) { return; }
        if (chapterRaw === '' || isNaN(parseInt(chapterRaw, 10))) { return; }
        if (verseRaw === '' || isNaN(parseInt(verseRaw, 10))) { return; }
    ...
    if (verses && Array.isArray(verses) && verses.length > 0) {
        verses.forEach(function (v) {
            if (v && typeof v.book === 'string' && typeof v.chapter === 'number' && typeof v.verse === 'number') {
