...
(function () {
  /* ---------------------------------------------------------------------------
     STATE (scoped inside IIFE to prevent cross-module variable collisions)
  --------------------------------------------------------------------------- */
  let _searchTerm = "";
  ...
  function _applySearchFilter(term, statusEl) {
    ...
    rows.forEach(function (row) {
      ...
      if (term.length === 0) {
        ...
        row.style.display = "";
        visibleCount++;
      } else if (_fuzzyMatch(term, title + " " + verse + " " + snippet)) {
        row.style.display = "";
        visibleCount++;
      } else {
        row.style.display = "none";
    ...
    if (statusEl) {
      if (term.length === 0) {
        ...
        statusEl.textContent =
          'No records match "' + term + '". Try a different search.';
      } else {
        statusEl.textContent =
...
----------------------------------------------------------------------------- */
  function _updateSearchCount() {
    ...
    if (statusEl) {
      if (visibleCount === 0) {
        statusEl.textContent =
          'No records match "' + _searchTerm + '". Try a different search.';
      } else {
        statusEl.textContent =
