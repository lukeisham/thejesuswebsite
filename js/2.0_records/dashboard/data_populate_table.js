...
function _renderRows(records) {
  ...
  records.forEach(function (record) {
    ...
    tr.setAttribute("data-snippet", (record.snippet || "").toLowerCase());
    ...
    const snippet = (record.snippet || record.description || "").substring(
      0,
      120,
    );
...
function _formatVerse(verseStr) {
  if (!verseStr) return "";
  try {
    const parsed = JSON.parse(verseStr);
    // primary_verse is stored as a JSON array, e.g. [{"book": "John", "chapter": 1, "verse": 14}]
    const v = Array.isArray(parsed) ? parsed[0] : parsed;
    if (v && v.book && v.chapter) {
