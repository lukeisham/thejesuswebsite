...
| 3 | Data/interpretation split | **+10** clear split; **−5** muddled; **0** one-sided; **0** unclassifiable | **Vector** (the section classifier) — **the dominant matrix.** Four stores (data bucket, close-analysis, interpretation bucket, linguistic register) label every body paragraph; a computed separation ratio decides the tier. Headings are not consulted. These labels **are** the section buckets for the entire rubric |<br>**Active** as of Signal 3 activation (2026-07-30). The close-analysis (Tier 2) store distinguishes synoptic comparison, Greek/Hebrew analysis, and text-critical evaluation from pure interpretation. See `Issues.md` #141 (resolved).
