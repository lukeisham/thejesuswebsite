...
**Source of truth for weights:** the weights table below is this document's own copy and is authoritative — do not duplicate it into another file. Do not run `rank_engine.py rescore` until this table has been brought into line with the actual code (`classifier/config.py`, `rank_engine.py`, `scripts/extract.js`) — a rescore against a stale table would score every article under the old rubric.
...
| 3 | Data/interpretation split | **+10** clear split; **−5** muddled; **0** one-sided; **0** unclassifiable | **LLM-labelled corpus** (primary — labels-corpus.json, 0.926 human-agreement; gate cleared) with **vector classifier** (secondary/cross-check — four stores: data bucket, close-analysis, interpretation bucket, linguistic register). A computed separation ratio decides the tier. Headings are not consulted. These labels **are** the section buckets for the entire rubric |<br>**Active** as of Signal 3 activation (2026-07-30); LLM labels activated as primary source 2026-07-31. The close-analysis (Tier 2) store distinguishes synoptic comparison, Greek/Hebrew analysis, and text-critical evaluation from pure interpretation. See `Issues.md` #141 and #163 (resolved).
...
| 10 | Literary analysis | **+6** for parable / teaching / Bible-book articles; **+4** for all other articles | **Vector** (the literary-analysis vector store) — single vector-embedding database trained on literary-analysis passages: narrative criticism, rhetorical devices (inclusio, chiasm, parallelism), genre conventions, intertextual allusion, reader-response, form-critical segmentation. Tiered by article category |<br>**Activated 2026-07-31**: vector family store (80 positive + 60 negative exemplars), t_fire=0.56, fires on ~19% of the corpus. +6/+4 tiered by category.
...
| 19 | Jesus seminar bias | **−3** per author, capped at **−6**; then **×2** if cited outside the interpretation sections / **×0.5** if interpretation-only; then a further **−2** if balanced debate (row 5) scored 0. Worst case **−14** | **Vector bias** (the Jesus Seminar vector store) — fixed list (Funk, Crossan, Borg) supplies the count; the section classifier supplies placement. No stance detection is attempted — placement and balance act as structural proxies (the stance-blind structural-proxy rationale) |
| 20 | OT–NT continuity criticism | **−3** per distinct critical pattern, capped at **−6** | **Vector bias** (the OT–NT discontinuity vector store) — same 7-dimension bias detection system (the supernatural-worldview 7-dimension vector system), applied to four schools (proof-texting, messianic divergence, Law abrogation, intertestamental evolution) + contradiction framing. Interpretation text included in scan (inverted section-awareness) |
| 21 | Mythicist bias | **−3** per author, capped at **−7**; then **×2** if cited outside the interpretation sections / **×0.5** if interpretation-only; then a further **−2** if balanced debate (row 5) scored 0. Worst case **−16** | **Vector bias** (the mythicist-framing vector store) — fixed list (Carrier, Price, Doherty) supplies the count; the section classifier supplies placement. No stance detection is attempted (the stance-blind structural-proxy rationale). Raised sensitivity on `is_passion` (the passion-sensitivity trigger, §2.9) |
| 22 | Criticism of the supernatural worldview | **−2** per instance, capped at **−8** | **Vector bias** (the supernatural-worldview 7-dimension vector system) — 7-dimension system split into embedding-detected markers (1, 2, 4, 7a) and computed metrics (3, 5, 6, 7b). Miracle- **and Passion-scoped**, section-aware (criticism-heading text excluded). Raised sensitivity on `is_passion` (the passion-sensitivity trigger, §2.9) |
| 23 | Secular-materialist presuppositions | **−2** per distinct term, capped at **−8** | **Vector bias** (the secular-materialist vector store) — same 7-dimension system (the supernatural-worldview 7-dimension vector system), own vector-embedding database. Miracle- **and Passion-scoped**, section-aware (criticism-heading text excluded). Raised sensitivity on `is_passion` (the passion-sensitivity trigger, §2.9). Placement multiplier does NOT apply |
| 24 | Wikipedia referencing | Tiered on `ref_count`: **−9** if 0 refs; **+3** if 1–4; **+1** if 5–9; **0** if 10+. Plus **−1**, independently, for poor referencing | **Unchanged plain lookup** (§3.8) — one signal spanning the whole reference-count spectrum, absorbing the former niche-exposure bonus. Zero references is a failure; *few* references on a genuinely niche topic is forgiven, so short well-sourced articles aren't punished by the count-based signals. The −1 poor-referencing penalty (DOM inspection for "citation needed" tags / maintenance banners) applies on top of whichever tier fires |
...
(see `ALGORITHM_GUIDE_the_how.md` §6.2 for the tie-break rule). Ties are expected; alphabetical ordering inside a
