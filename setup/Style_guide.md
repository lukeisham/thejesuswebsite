...
- **Page header**: `h1` category title (e.g. "Parables", "External Witnesses"), brief description.
- **Category navigation**: a horizontal row of ghost-style chip links above the header, one per resource list page, linking between categories. Active chip: `1px solid var(--accent)` border, `--bg-surface-alt` background.
...
- `logout.html`: minimal confirmation card with a "Sign in again" link — same centred card, no form.
