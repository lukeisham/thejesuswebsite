...
One row per (article, signal) — 28 rows per published article. `signal_key` matches keys in the static `SIGNAL_DICTIONARY` (`frontend/assets/js/utils/wikipedia-signals.js`). `contribution` is points earned (negative for negative signals); `cap` is that signal's max magnitude for this article (also negative for negative signals).
