...
7. **Admin pages require passkey auth the agent cannot perform.** The admin uses WebAuthn passkey sign-in, which needs the user's device authenticator — no agent can automate it (see `setup/Issues.md` #33/#76 for history). Two facts drive the procedure below:
   ...
   **The one reliable flow — agent opens the tab, user logs in once, agent drives it:**
   1. **Pause and tell the user you're about to open an admin tab for them to authenticate.** Do not proceed silently.
   2. Create a fresh tab in the agent's own group (`tabs_context_mcp {createIfEmpty:true}`, or `preview_start` for the Browser pane) and `navigate` it to the admin page — it will redirect to `/admin/auth/login.html`. (The pre-redirect flash of admin UI in a first screenshot is NOT proof of a session.)
   ...
   4. Re-navigate to the target admin page and drive the test in that now-authenticated tab (verify via `javascript_tool`/`read_page` DOM queries + `read_console_messages`, per steps 4–5 above).
